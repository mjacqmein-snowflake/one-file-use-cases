...
SET OTIF_SOURCE_DISCOVERY_MODE = 'AUTO';
SET OTIF_SOURCE_DISCOVERY_SCHEMA = '';
SET OTIF_SOURCE_DISCOVERY_AI_APPROVED = TRUE;
SET OTIF_SOURCE_DISCOVERY_MODEL = 'claude-sonnet-4-6';
SET OTIF_SOURCE_DISCOVERY_N = 0;
SET OTIF_SOURCE_DISCOVERY_1 = '';
SET OTIF_SOURCE_DISCOVERY_2 = '';
SET OTIF_SOURCE_DISCOVERY_3 = '';
SET OTIF_SOURCE_DISCOVERY_4 = '';
...
SET OTIF_APPROVE = TRUE;
SET OTIF_VERBOSE_OUTPUT = FALSE;
...
BEGIN
  ...
  LET source_slots OBJECT := OBJECT_CONSTRUCT(
    'OTIF_SOURCE_PO', TRIM($OTIF_SOURCE_PO::VARCHAR),
    'OTIF_SOURCE_RECEIPTS', TRIM($OTIF_SOURCE_RECEIPTS::VARCHAR),
    'OTIF_SOURCE_VENDORS', TRIM($OTIF_SOURCE_VENDORS::VARCHAR));
  LET source_configured INTEGER := (SELECT COUNT(*) FROM TABLE(FLATTEN(INPUT => :source_slots)) WHERE VALUE::VARCHAR <> '');
  LET source_discovery_mode VARCHAR := UPPER($OTIF_SOURCE_DISCOVERY_MODE::VARCHAR);
  ...
  LET initial_discovery BOOLEAN := :source_discovery_mode = 'AUTO' AND $OTIF_APPROVE::BOOLEAN;
  IF (:mode <> 'SAMPLE' AND (:source_configured < ARRAY_SIZE(OBJECT_KEYS(:source_slots)) OR :source_invalid > 0 OR :source_discovery_mode IN ('INVENTORY', 'PROPOSE'))) THEN
    LET discovery_scope VARCHAR := UPPER(TRIM($OTIF_SOURCE_DISCOVERY_SCHEMA::VARCHAR));
    LET discovery_own VARCHAR := UPPER($OTIF_SCHEMA::VARCHAR);
    ...
    LET discovery_note VARCHAR := 'Metadata only. Review the inventory. To request one bounded AI proposal, set OTIF_SOURCE_DISCOVERY_MODE = PROPOSE and OTIF_SOURCE_DISCOVERY_AI_APPROVED = TRUE. AI tokens and warehouse work are billable; no source rows or objects are changed.';
    BEGIN
      ...
      ELSEIF (:db IS NULL OR NOT REGEXP_LIKE(:db, '[A-Za-z_][A-Za-z0-9_$]*') OR (:discovery_scope <> '' AND NOT REGEXP_LIKE(:discovery_scope, '[A-Z_][A-Z0-9_$]*'))) THEN
        ...
        discovery_note := 'Select a database and optionally set OTIF_SOURCE_DISCOVERY_SCHEMA to an exact unquoted schema name.';
      ELSE
        ...
        ELSE
          ...
          LET inventory_query VARCHAR := 'WITH relations AS (SELECT t.TABLE_CATALOG AS DB, t.TABLE_SCHEMA AS SCH, t.TABLE_NAME AS TAB, t.TABLE_TYPE AS KIND, '
            ...
            || 'MAX(IFF(ARRAY_CONTAINS((t.TABLE_CATALOG||''.''||t.TABLE_SCHEMA||''.''||t.TABLE_NAME)::VARIANT,PARSE_JSON(?)),1000,0)) + MAX(IFF(REGEXP_LIKE(LOWER(t.TABLE_NAME), ''.*(otif|receipts|vendor|vendors).*''),10,0)) + SUM(IFF(REGEXP_LIKE(LOWER(c.COLUMN_NAME), ''.*(otif|receipts|vendor|vendors).*''),1,0)) AS RELEVANCE '
              ...
              discovery_proposal := (SELECT AI_COMPLETE(model => :discovery_model, prompt => :discovery_prompt,
                ...
                response_format => {'type':'json','schema':{'type':'object','additionalProperties':false,
                  'properties':{'mappings':{'type':'array','items':{'type':'object','additionalProperties':false,
                    'properties':{'setting':{'type':'string','enum':['OTIF_SOURCE_PO','OTIF_SOURCE_RECEIPTS','OTIF_SOURCE_VENDORS']},'table':{'type':'string'},'columns':{'type':'array','items':{'type':'string'}},'reason':{'type':'string'}},
    ...
    WHILE (:discovery_chunk < :discovery_chunks) DO
      EXECUTE IMMEDIATE 'SET OTIF_SOURCE_DISCOVERY_' || (:discovery_chunk + 1) || ' = ''' || SUBSTR(:discovery_encoded,:discovery_chunk*12000+1,12000) || '''';
      discovery_chunk := :discovery_chunk + 1;
    END WHILE;
    EXECUTE IMMEDIATE 'SET OTIF_SOURCE_DISCOVERY_N = ' || :discovery_chunks;
...
DECLARE
  res RESULTSET;
BEGIN
    IF ($OTIF_SOURCE_DISCOVERY_N::INTEGER > 0) THEN
    LET source_handoff VARCHAR := $OTIF_SOURCE_DISCOVERY_1 || $OTIF_SOURCE_DISCOVERY_2 || $OTIF_SOURCE_DISCOVERY_3 || $OTIF_SOURCE_DISCOVERY_4;
    LET source_result VARIANT := PARSE_JSON(BASE64_DECODE_STRING(:source_handoff));
    IF (UPPER($OTIF_SOURCE_DISCOVERY_MODE::VARCHAR) <> 'AUTO' OR :source_result:status::VARCHAR IN ('INVALID_SOURCE_SETTING','INVALID_SCOPE')) THEN
...
|| 'before undoing it again.''; '
 ...
 || 'BASE64_DECODE_STRING(''' || BASE64_ENCODE(LEFT(:review_raw, 8000)) || ''')');
  ...
  IF (:workload_blocked) THEN
    IF (:approved AND 'VENDOR_OTIF_APP' <> '' AND 'sc1_vendor_otif' <> '24_voice_of_customer' AND :app_build_end >= :app_build_start) THEN
  ...
  IF (NOT COALESCE(:approved, FALSE)) THEN
    IF (NOT $OTIF_VERBOSE_OUTPUT::BOOLEAN) THEN
      res := (SELECT IFF(:hard_block <> '' OR (:review_verdict = 'DO_NOT_PROCEED' AND NOT :override_asked), 'BLOCKED', 'READY_TO_BUILD') AS STATUS,
        ...
        IFF(:hard_block <> '', :hard_block, IFF(:review_verdict = 'DO_NOT_PROCEED' AND NOT :override_asked, TO_JSON(:review_findings), 'Review the cost and discovery packet, then set OTIF_APPROVE = TRUE and rerun. Set OTIF_VERBOSE_OUTPUT = TRUE for the full plan.')) AS NEXT_ACTION,
  ...
  LET receipt_app_name STRING := 'VENDOR_OTIF_APP';
  ...
  IF (:receipt_app_name <> '' AND :app_build_end >= :app_build_start AND :receipt_app_statements = :app_build_end - :app_build_start + 1) THEN
    ...
    IF (:receipt_app_exists) THEN
      BEGIN
        ...
        receipt_workspace_exists := (SELECT COUNT(*) = 1 FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())) WHERE "name" = 'ONESHOT_SOURCE' AND "comment" = 'oneshot-source:sc1_vendor_otif');
      EXCEPTION WHEN OTHER THEN
