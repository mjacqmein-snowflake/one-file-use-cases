...
-- guarantee.
SET COSTEFF_APP_SLEEP_MINUTES = 5;
...
|| 'before undoing it again.''; '
 ...
 || 'BASE64_DECODE_STRING(''' || BASE64_ENCODE(LEFT(:review_raw, 8000)) || ''')');
  ...
  IF (:workload_blocked) THEN
    IF (:approved AND 'COSTEFF_APP' <> '' AND '11_cost_efficiency' <> '24_voice_of_customer' AND :app_build_end >= :app_build_start) THEN
