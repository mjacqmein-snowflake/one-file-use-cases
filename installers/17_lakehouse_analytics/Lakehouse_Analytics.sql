...
|| 'before undoing it again.''; '
 ...
 || 'BASE64_DECODE_STRING(''' || BASE64_ENCODE(LEFT(:review_raw, 8000)) || ''')');
  ...
  IF (:workload_blocked) THEN
    IF (:approved AND 'LAKEHOUSE_ANALYTICS_APP' <> '' AND '17_lakehouse_analytics' <> '24_voice_of_customer' AND :app_build_end >= :app_build_start) THEN
