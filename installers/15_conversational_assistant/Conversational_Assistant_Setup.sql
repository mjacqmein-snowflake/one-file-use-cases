...
-- guarantee.
SET CONVO_APP_SLEEP_MINUTES = 240;

-- How far back discovery and the views look.
SET CONVO_WINDOW_DAYS = 14;
...
SET CONVO_DEPLOY_TIER = 'DISCOVER';
...
SET CONVO_OUTPUT_TOKEN_RATIO = 0.5;
...
--
-- DO_NOT_PROCEED closes the gate even when CONVO_APPROVE is TRUE. Setting this to
...
SET CONVO_OVERRIDE_REVIEW = FALSE;
...
SET CONVO_NOTIFICATION_INTEGRATION = '';
...
SET CONVO_ALLOW_SAMPLE_ACTIONS = TRUE;
...
SET CONVO_MODEL = 'claude-opus-5';

-- Internal. Do not edit. Block 1 publishes its findings here in chunks, because
-- one session variable caps at 16,384 bytes.
SET CONVO_SIGNALS_N = 0;

-- ── Source table ───────────────────────────────────────────────────────────────
-- BLANK MEANS NOTHING HAPPENS. Set to a fully-qualified table name to enable.
SET CONVO_SOURCE_TABLE = '';

-- ── Chat model ────────────────────────────────────────────────────────────────
-- The model used for answering questions (bulk, per-question). SEPARATE from
-- CONVO_MODEL which is the judgement/review model in the shared template.
-- Smaller models are cheaper per question; larger are more accurate.
SET CONVO_CHAT_MODEL = 'claude-4-sonnet';

-- ── Holdout evaluation ────────────────────────────────────────────────────────
-- Ground truth table with pre-computed answers for scoring.
-- If blank, the build derives ground truth from the source table directly.
SET CONVO_GROUND_TRUTH_TABLE = '';

-- Maximum holdout questions to evaluate per run. Controls AI spend.
SET CONVO_MAX_EVAL_QUESTIONS = 10;

-- ── Profile ───────────────────────────────────────────────────────────────────
-- Whether to run the column profile (reads business data).
-- NOTE: the shared template already provides SET CONVO_PROFILE = FALSE as a
-- default. Declaring it again here would create a duplicate that the gauntlet's
-- override_settings (count=1) cannot reach. Removed to avoid that collision.
...
BEGIN
  ...
  LET wh   STRING := COALESCE(NULLIF($CONVO_APP_WAREHOUSE::VARCHAR, ''), CURRENT_WAREHOUSE());
  LET sch  STRING := $CONVO_SCHEMA::VARCHAR;
  LET mode STRING := UPPER(COALESCE($CONVO_MODE::VARCHAR, 'DISCOVER'));
  ...
  BEGIN
    LET probe STRING := (SELECT SNOWFLAKE.CORTEX.AI_COMPLETE(
      COALESCE(NULLIF($CONVO_MODEL::VARCHAR, ''), 'claude-opus-5'), 'Reply with OK.'));
    cortex_ok := TRUE;
  ...
  LET tier      STRING := UPPER(COALESCE(NULLIF($CONVO_DEPLOY_TIER::VARCHAR, ''), 'DISCOVER'));
  ...
  LET ni       STRING := COALESCE(NULLIF($CONVO_NOTIFICATION_INTEGRATION::VARCHAR, ''), '');
  LET ni_found INT    := 0;
  LET ni_avail INT    := 0;
  BEGIN
  ...
  BEGIN
    profile_on := (SELECT TRY_CAST($CONVO_PROFILE::VARCHAR AS BOOLEAN));
  EXCEPTION WHEN OTHER THEN profile_on := FALSE;
  END;
  LET cap NUMBER(38,2) := COALESCE((SELECT TRY_CAST($CONVO_CREDIT_CAP::VARCHAR AS NUMBER)), 0);


  LET approved BOOLEAN := FALSE;
  BEGIN
    approved := (SELECT TRY_CAST($CONVO_APPROVE::VARCHAR AS BOOLEAN));
  ...
  res := (
    SELECT 1 AS step, 'TARGET DATABASE' AS check_name,
           ...
           IFF(:db IS NULL, 'Run USE DATABASE, or set CONVO_TARGET_DB.',
    ...
    UNION ALL SELECT 3, 'WAREHOUSE', COALESCE(:wh, 'NONE SELECTED'),
           IFF(:wh IS NULL, 'Run USE WAREHOUSE, or set CONVO_APP_WAREHOUSE.', '')
    ...
    UNION ALL SELECT 5, 'CORTEX (' || COALESCE(NULLIF($CONVO_MODEL::VARCHAR, ''), 'claude-opus-5')
    ...
    UNION ALL SELECT 8, 'GATE', IFF(:approved, 'OPEN — Block 3 will build', 'CLOSED — nothing will be created'),
           IFF(:approved, 'Review the plan below before you let this run.', 'To build: set CONVO_APPROVE = TRUE and run the file again.')
    UNION ALL SELECT 9, 'DEPLOY TIER', :tier,
           CASE :tier
             WHEN 'DISCOVER' THEN 'Costs below are ARITHMETIC ESTIMATES. Nothing is measured at this tier. Set CONVO_DEPLOY_TIER = ''LIMITED'' to get a real number.'
    ...
    UNION ALL SELECT 10, 'PROFILE', IFF(:profile_on, 'ON — will sample the columns the plan uses',
           ...
           IFF(:profile_on,
               ...
               'This is the gap that lets a plan build on a column that exists and is empty. Set CONVO_PROFILE = TRUE to close it. The review will return CAVEAT rather than PROCEED while it is off.')
...
DECLARE
  res RESULTSET;
BEGIN
  LET w    INT    := COALESCE((SELECT TRY_CAST($CONVO_WINDOW_DAYS::VARCHAR AS INT)), 14);
  LET db   STRING := COALESCE(NULLIF($CONVO_TARGET_DB::VARCHAR, ''), CURRENT_DATABASE());
  LET mode STRING := UPPER(COALESCE($CONVO_MODE::VARCHAR, 'DISCOVER'));
  ...
  LET table_columns ARRAY := ARRAY_CONSTRUCT();
  LET numeric_cols ARRAY := ARRAY_CONSTRUCT();
  ...
  ELSE
    BEGIN
      EXECUTE IMMEDIATE 'SELECT COUNT(*) AS N FROM ' || :cv_table;
      table_row_count := (SELECT N FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())));
      sig := OBJECT_INSERT(:sig, 'source_table', IFF(:table_row_count > 0, 'AVAILABLE', 'EMPTY'), TRUE);
    ...
    -- Enumerate columns and classify by type
    BEGIN
      EXECUTE IMMEDIATE
        'SELECT COLUMN_NAME, DATA_TYPE FROM '
     || SPLIT_PART(:cv_table, '.', 1) || '.INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '''
     || SPLIT_PART(:cv_table, '.', 2) || ''' AND TABLE_NAME = '''
     || SPLIT_PART(:cv_table, '.', 3) || ''' ORDER BY ORDINAL_POSITION';
      LET col_cursor CURSOR FOR SELECT "COLUMN_NAME", "DATA_TYPE" FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()));
      FOR rec IN col_cursor DO
        table_columns := ARRAY_APPEND(:table_columns, rec."COLUMN_NAME");
        table_col_count := :table_col_count + 1;
        IF (rec."DATA_TYPE" IN ('NUMBER', 'DECIMAL', 'NUMERIC', 'INT', 'INTEGER',
                                'BIGINT', 'SMALLINT', 'TINYINT', 'FLOAT', 'DOUBLE',
                                'REAL', 'FIXED')) THEN
          numeric_cols := ARRAY_APPEND(:numeric_cols, rec."COLUMN_NAME");
        ELSEIF (rec."DATA_TYPE" IN ('VARCHAR', 'STRING', 'TEXT', 'CHAR')) THEN
          text_cols := ARRAY_APPEND(:text_cols, rec."COLUMN_NAME");
        END IF;
      END FOR;
    EXCEPTION WHEN OTHER THEN
      table_col_count := 0;
  ...
  sig := OBJECT_INSERT(:sig, 'table_columns', :table_col_count, TRUE);
  cnt := OBJECT_INSERT(:cnt, 'table_columns', :table_col_count, TRUE);

  -- ── Probe: Cortex availability ─────────────────────────────────────────────
  BEGIN
    LET p STRING := (SELECT AI_COMPLETE(:cv_model, 'Reply OK.'));
  ...
  -- ── Probe: ground truth table ──────────────────────────────────────────────
  IF (:cv_gt IS NOT NULL) THEN
    BEGIN
      EXECUTE IMMEDIATE 'SELECT COUNT(*) AS N FROM ' || :cv_gt;
      LET gn INT := (SELECT N FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())));
      sig := OBJECT_INSERT(:sig, 'ground_truth', 'AVAILABLE', TRUE);
      cnt := OBJECT_INSERT(:cnt, 'ground_truth', :gn, TRUE);
    EXCEPTION WHEN OTHER THEN
      sig := OBJECT_INSERT(:sig, 'ground_truth', 'NO ACCESS', TRUE);
      cnt := OBJECT_INSERT(:cnt, 'ground_truth', 0, TRUE);
    END;
  ELSE
    sig := OBJECT_INSERT(:sig, 'ground_truth', 'NOT CONFIGURED', TRUE);
    cnt := OBJECT_INSERT(:cnt, 'ground_truth', 0, TRUE);
  END IF;
  --
  ...
  LET payload STRING := TO_JSON(OBJECT_CONSTRUCT(
      ...
      , 'table_row_count', COALESCE(GET(:cnt, 'source_table')::NUMBER, 0)
      , 'table_col_count', COALESCE(GET(:cnt, 'table_columns')::NUMBER, 0)
  ));
  ...
  WHILE (:ci < :nchunks) DO
    ...
    EXECUTE IMMEDIATE 'SET CONVO_SIGNALS_' || (:ci + 1)
                   || ' = ''' || :piece || '''';
    ci := :ci + 1;
  END WHILE;
  EXECUTE IMMEDIATE 'SET CONVO_SIGNALS_N = ' || :nchunks;

  -- Prove the handoff survived rather than assuming it did.
  IF ((SELECT COALESCE(TRY_CAST(GETVARIABLE('CONVO_SIGNALS_N') AS INT), 0)) <> :nchunks) THEN
...
BEGIN
  ...
  BEGIN
    prof_on := (SELECT TRY_CAST($CONVO_PROFILE::VARCHAR AS BOOLEAN));
...
-- The profile reads the source table columns the chat will reason over.
-- Without profiling these, the plan has no evidence that the data the model
-- will be asked about actually contains usable values.
--
-- Named explicitly: the shared profile block validates every declared column
-- against INFORMATION_SCHEMA and drops missing ones, so a literal '*' leaves
-- the good-column list EMPTY and nothing is profiled at all. These are the
-- columns the holdout questions aggregate over.
LET p_table STRING := COALESCE(NULLIF($CONVO_SOURCE_TABLE::VARCHAR, ''), '');
...
IF (:p_table <> '') THEN
  targets := ARRAY_APPEND(:targets, OBJECT_CONSTRUCT(
    ...
    'columns', ARRAY_CONSTRUCT(
        ...
        'AMOUNT', 'QUANTITY', 'ORDER_DATE', 'REGION', 'STATUS'),
    'grain', 'ORDER_ID'));
END IF;
  ...
  IF (NOT :prof_on) THEN
    res := (SELECT 'PROFILE NOT RUN' AS target_table, '' AS column_name, '' AS data_type,
                   ...
                   'Set CONVO_PROFILE = TRUE to check whether the columns this plan '
        ...
        'note', CASE
                  ...
                  WHEN :fill < :min_fill
                    THEN 'Populated ' || :fill || '% of the time, under the '
                      || :min_fill || '% floor set by CONVO_MIN_FILL_PCT.'
  ...
  WHILE (:pi < :nchunks) DO
    ...
    EXECUTE IMMEDIATE 'SET CONVO_PROFILE_' || (:pi + 1) || ' = ''' || :piece || '''';
    pi := :pi + 1;
  END WHILE;
  EXECUTE IMMEDIATE 'SET CONVO_PROFILE_N = ' || :nchunks;
...
BEGIN
  ...
  LET nchunks INT := COALESCE((SELECT TRY_CAST(GETVARIABLE('CONVO_SIGNALS_N') AS INT)), 0);
  IF (:nchunks = 0) THEN
  ...
  LET buf STRING :=
       COALESCE(GETVARIABLE('CONVO_SIGNALS_1'), '')
    || COALESCE(GETVARIABLE('CONVO_SIGNALS_2'), '')
    || COALESCE(GETVARIABLE('CONVO_SIGNALS_3'), '')
    || COALESCE(GETVARIABLE('CONVO_SIGNALS_4'), '')
    || COALESCE(GETVARIABLE('CONVO_SIGNALS_5'), '')
    || COALESCE(GETVARIABLE('CONVO_SIGNALS_6'), '')
    || COALESCE(GETVARIABLE('CONVO_SIGNALS_7'), '')
    || COALESCE(GETVARIABLE('CONVO_SIGNALS_8'), '');
  ...
  LET sch    STRING  := $CONVO_SCHEMA::VARCHAR;
  LET wh     STRING  := COALESCE(NULLIF($CONVO_APP_WAREHOUSE::VARCHAR, ''), CURRENT_WAREHOUSE());
  LET budget NUMBER  := COALESCE((SELECT TRY_CAST($CONVO_BUDGET_CREDITS::VARCHAR AS NUMBER)), 0);
  ...
  LET pchunks INT := COALESCE((SELECT TRY_CAST(GETVARIABLE('CONVO_PROFILE_N') AS INT)), 0);
  LET prof        VARIANT := NULL;
  LET prof_status STRING  := 'NOT RUN';
  IF (:pchunks > 0) THEN
    LET pbuf STRING :=
         COALESCE(GETVARIABLE('CONVO_PROFILE_1'), '')
      || COALESCE(GETVARIABLE('CONVO_PROFILE_2'), '')
      || COALESCE(GETVARIABLE('CONVO_PROFILE_3'), '')
      || COALESCE(GETVARIABLE('CONVO_PROFILE_4'), '');
  ...
  LET run_id STRING := COALESCE(NULLIF($CONVO_RUN_ID::VARCHAR, ''), UUID_STRING());
  LET tier   STRING := UPPER(COALESCE(NULLIF($CONVO_DEPLOY_TIER::VARCHAR, ''), 'DISCOVER'));
  IF (:tier NOT IN ('DISCOVER', 'LIMITED', 'PRODUCTION')) THEN
    tier := 'DISCOVER';
  END IF;
  LET qtag STRING := TO_JSON(OBJECT_CONSTRUCT(
      'oneshot', 'Conversational Assistant', 'prefix', 'CONVO', 'run_id', :run_id, 'tier', :tier));
  ...
  LET meas_wh STRING := COALESCE(NULLIF($CONVO_MEASURE_WAREHOUSE::VARCHAR, ''),
                                 LEFT(:sch, 80) || '_ONESHOT_WH');
  LET credit_cap NUMBER(38,2) := COALESCE((SELECT TRY_CAST($CONVO_CREDIT_CAP::VARCHAR AS NUMBER)), 0);
  LET rate NUMBER(38,4) := COALESCE((SELECT TRY_CAST($CONVO_COST_PER_CREDIT::VARCHAR AS NUMBER)), 3);
  LET out_ratio NUMBER(38,4) := COALESCE((SELECT TRY_CAST($CONVO_OUTPUT_TOKEN_RATIO::VARCHAR AS NUMBER)), 0.5);
  LET min_fill NUMBER(38,2) := COALESCE((SELECT TRY_CAST($CONVO_MIN_FILL_PCT::VARCHAR AS NUMBER)), 60);
  LET notif STRING := COALESCE(NULLIF($CONVO_NOTIFICATION_INTEGRATION::VARCHAR, ''), '');
  ...
  IF (:db IS NULL) THEN
    res := (SELECT 0 AS step, 'BLOCKED' AS action,
                   'No database selected. Run USE DATABASE or set CONVO_TARGET_DB.' AS statement);
    RETURN TABLE(res);
  END IF;
  IF (:wh IS NULL) THEN
    res := (SELECT 0 AS step, 'BLOCKED' AS action,
                   'No warehouse selected. Run USE WAREHOUSE or set CONVO_APP_WAREHOUSE.' AS statement);
    RETURN TABLE(res);
  END IF;
  ...
  LET allow_actions BOOLEAN := COALESCE(
    (SELECT TRY_CAST($CONVO_ALLOW_ACTIONS::VARCHAR AS BOOLEAN)), FALSE);
  ...
  -- because a build produced by an OLDER file that has no CONVO_ALLOW_SAMPLE_ACTIONS
  -- line should still get the new default rather than silently disarming.
  LET allow_sample_actions BOOLEAN := COALESCE(
    (SELECT TRY_CAST($CONVO_ALLOW_SAMPLE_ACTIONS::VARCHAR AS BOOLEAN)), TRUE);
  ...
  LET adapt_model  STRING  := COALESCE(NULLIF($CONVO_MODEL::VARCHAR, ''), 'claude-opus-5');
  ...
  LET warm_on BOOLEAN := COALESCE(
    (SELECT TRY_CAST($CONVO_KEEP_APP_WARM::VARCHAR AS BOOLEAN)), FALSE);
  LET warm_wh STRING := UPPER(TRIM(COALESCE(
    NULLIF($CONVO_WARM_WAREHOUSE::VARCHAR, ''), 'ONESHOT_APP_WH')));
  ...
  LET wh_named BOOLEAN := (NULLIF($CONVO_APP_WAREHOUSE::VARCHAR, '') IS NOT NULL);
  LET warm_status STRING := 'OFF';

  IF (:warm_on AND :wh_named) THEN
    warm_status := 'DECLINED_EXPLICIT_WAREHOUSE';
    notes := ARRAY_APPEND(:notes,
      'APP WARMING SKIPPED: CONVO_APP_WAREHOUSE names ' || :wh || ' explicitly, so '
   || 'the app stays there rather than being moved to ' || :warm_wh || '. Clear '
   || 'CONVO_APP_WAREHOUSE to let warming manage the app warehouse, or set '
  ...
  ELSEIF (:warm_on AND :wh_ok) THEN
   ...
   || 'because they all share this warehouse. Set CONVO_KEEP_APP_WARM = FALSE to '
  ...
  ELSEIF (:warm_on) THEN
    ...
    notes := ARRAY_APPEND(:notes,
      'APP WARMING DEGRADED: CONVO_KEEP_APP_WARM is TRUE but ' || CURRENT_ROLE()
  ...
  LET sleep_min INT := COALESCE(
    (SELECT TRY_CAST($CONVO_APP_SLEEP_MINUTES::VARCHAR AS INT)), 240);
 ...
 || 'RUN_BY VARCHAR DEFAULT CURRENT_USER(), TEARDOWN_AT TIMESTAMP_NTZ)');
  ...
  IF (:tier IN ('LIMITED', 'PRODUCTION') AND :wh_ok) THEN
   ...
   || 'COMMENT = ''oneshot Conversational Assistant run ' || :run_id || ' - dropped by TEARDOWN''');
 ...
 || '''' || :tgt || ''' AS BUILT_IN, '
 || 'CURRENT_TIMESTAMP() AS BUILT_AT, ''Conversational Assistant'' AS SOLUTION, '
 ...
 || '''CONVO'' AS SETTING_PREFIX');
  ...
  ELSE
   ...
   || 'QUESTION_CLASS VARCHAR, DERIVATION_SQL VARCHAR)');
    ...
    IF (:cv_gt IS NOT NULL) THEN
     ...
     || '''distinct'', ''SELECT COUNT(DISTINCT CUSTOMER_NAME) FROM source'' FROM ' || :cv_table);
      stmts := ARRAY_APPEND(:stmts,
        'INSERT INTO ' || :tgt || '.HOLDOUT_QUESTIONS '
     || 'SELECT 4, ''How many distinct regions are there?'', '
     || 'COUNT(DISTINCT REGION)::VARCHAR, '
     || '''distinct'', ''SELECT COUNT(DISTINCT REGION) FROM source'' FROM ' || :cv_table);
      stmts := ARRAY_APPEND(:stmts,
        'INSERT INTO ' || :tgt || '.HOLDOUT_QUESTIONS '
     || 'SELECT 5, ''What is the average order amount?'', '
     || 'ROUND(AVG(AMOUNT), 2)::VARCHAR, '
     || '''average'', ''SELECT AVG(AMOUNT) FROM source'' FROM ' || :cv_table);
      stmts := ARRAY_APPEND(:stmts,
        'INSERT INTO ' || :tgt || '.HOLDOUT_QUESTIONS '
     || 'SELECT 6, ''How many orders have status completed?'', '
     || 'COUNT_IF(LOWER(STATUS) = ''completed'')::VARCHAR, '
     || '''filtered_count'', ''SELECT COUNT(*) FROM source WHERE STATUS=completed'' FROM ' || :cv_table);
      stmts := ARRAY_APPEND(:stmts,
        'INSERT INTO ' || :tgt || '.HOLDOUT_QUESTIONS '
     || 'SELECT 7, ''How many distinct product categories are there?'', '
     || 'COUNT(DISTINCT PRODUCT_CATEGORY)::VARCHAR, '
     || '''distinct'', ''SELECT COUNT(DISTINCT PRODUCT_CATEGORY) FROM source'' FROM ' || :cv_table);
    ELSE
     ...
     || '''count'', ''SELECT COUNT(*) FROM source'' FROM ' || :cv_table);
      stmts := ARRAY_APPEND(:stmts,
        'INSERT INTO ' || :tgt || '.HOLDOUT_QUESTIONS '
     || 'SELECT 2, ''What is the total revenue?'', ROUND(SUM(AMOUNT),2)::VARCHAR, '
     || '''sum'', ''SELECT SUM(AMOUNT) FROM source'' FROM ' || :cv_table);
    END IF;
   ...
   || '  q_cur CURSOR FOR SELECT QUESTION_ID, QUESTION, EXPECTED_ANSWER, QUESTION_CLASS '
   ...
   || '      v_norm := '' '' || UPPER(REGEXP_REPLACE(:v_gen, ''[^A-Za-z0-9_]'', '' '')) || '' ''; '
   || '      IF (NOT (LTRIM(UPPER(:v_gen)) LIKE ''SELECT%'' '
   || '            OR LTRIM(UPPER(:v_gen)) LIKE ''WITH%'')) THEN '
   || '        v_status := ''UNSCORABLE''; '
   || '        v_answer := ''MODEL DID NOT RETURN SQL: '' || LEFT(COALESCE(:v_gen, ''''), 200); '
   || '      ELSEIF (:v_norm LIKE ''% DROP %'' OR :v_norm LIKE ''% DELETE %'' '
   || '           OR :v_norm LIKE ''% UPDATE %'' OR :v_norm LIKE ''% INSERT %'' '
   || '           OR :v_norm LIKE ''% MERGE %''  OR :v_norm LIKE ''% CREATE %'' '
   || '           OR :v_norm LIKE ''% ALTER %''  OR :v_norm LIKE ''% GRANT %'' '
   || '           OR :v_norm LIKE ''% REVOKE %'' OR :v_norm LIKE ''% TRUNCATE %'' '
   || '           OR :v_norm LIKE ''% CALL %''   OR :v_norm LIKE ''% COPY %'' '
   || '           OR :v_norm LIKE ''% EXECUTE %'') THEN '
   || '        v_status := ''REFUSED_UNSAFE_SQL''; '
   || '        v_answer := ''REFUSED, generated SQL contained a write or DDL keyword''; '
   || '      ELSE '
...
-- ── POC SUCCESS CRITERIA ──────────────────────────────────────────────────────
-- What would make this Conversational Assistant POC a success, measured against
-- bars derived from THIS account rather than from a demo.
--
-- EVERY CRITERION IS GATED ON THE SLOT IT READS. When CONVO_SOURCE_TABLE is
-- blank, nothing is built. When Cortex is unavailable, the holdout evaluation
-- is skipped and accuracy criteria are not declared.
--
-- WHAT IS DELIBERATELY NOT HERE. There is no "user satisfaction" or "adoption
-- rate" criterion. Both need production traffic from real users over time, and
-- this build runs once. The holdout evaluation measures factual accuracy on
-- single-aggregation questions; it does not measure explanation quality,
-- multi-step reasoning, hallucination rate, or usefulness.

-- ── Completeness: every holdout question was evaluated ───────────────────────
IF (:cv_table IS NOT NULL AND :sig:cortex::STRING = 'AVAILABLE') THEN
  success_criteria := ARRAY_APPEND(:success_criteria, OBJECT_CONSTRUCT(
    'code', 'CONVO_HOLDOUT_EVALUATED',
    'label', 'Every seeded holdout question was evaluated against its expected answer',
    'why', 'A holdout question that was seeded but not evaluated is a gap in the '
        || 'accuracy measurement. If AI_COMPLETE fails on some questions and succeeds '
        || 'on others, the accuracy rate describes a cherry-picked subset.',
    'compare', '=',
    'units', 'questions evaluated',
    'basis', 'BY_QUERY_ID',
    'target_sql', 'SELECT COUNT(*) FROM ' || :tgt || '.HOLDOUT_QUESTIONS',
    'actual_sql', 'SELECT COUNT(DISTINCT QUESTION_ID) FROM ' || :tgt
        || '.HOLDOUT_RESULTS',
    'target_derivation', 'The number of holdout questions seeded at build time. '
        || 'Every one should have a corresponding result row.'));

  -- ── Accuracy: the assistant gets at least half right ────────────────────────
  -- 50% is the minimum bar. Below it answers are more often wrong than right
  -- and the assistant is worse than a coin flip.
  success_criteria := ARRAY_APPEND(:success_criteria, OBJECT_CONSTRUCT(
    'code', 'CONVO_ACCURACY',
    'label', 'Holdout accuracy is at least 50% on factual single-aggregation questions',
    'why', 'Below 50% the assistant is more often wrong than right on questions '
        || 'with deterministic SQL-derivable answers. A user who trusts it would be '
        || 'worse off than one who ran the SQL themselves.',
    'compare', '>=',
    'units', 'percent correct',
    'basis', 'BY_QUERY_ID',
    'target_sql', 'SELECT 50',
    'actual_sql', 'SELECT CASE WHEN SUM(EVALUATED) = 0 THEN NULL '
        || 'ELSE ROUND(100.0 * SUM(CORRECT) / NULLIF(SUM(EVALUATED), 0), 1) '
        || 'END FROM ' || :tgt || '.V_HOLDOUT_ACCURACY',
    'target_derivation', '50% accuracy. This is our judgement about the minimum '
        || 'useful bar for a conversational assistant: below it, answers are more '
        || 'often wrong than right. You can argue with the number.'));

  -- ── Logging: every question asked is logged ────────────────────────────────
  success_criteria := ARRAY_APPEND(:success_criteria, OBJECT_CONSTRUCT(
    'code', 'CONVO_LOGGING_ACTIVE',
    'label', 'The chat log captured every holdout evaluation question',
    'why', 'An assistant without a log cannot be audited, and an accuracy claim '
        || 'without evidence of what was asked is unverifiable. The log must '
        || 'capture at least the holdout questions that were evaluated.',
    'compare', '>=',
    'units', 'logged questions',
    'basis', 'BY_QUERY_ID',
    'target_sql', 'SELECT COUNT(DISTINCT QUESTION_ID) FROM ' || :tgt
        || '.HOLDOUT_RESULTS',
    'actual_sql', 'SELECT COUNT(*) FROM ' || :tgt || '.CHAT_LOG',
    'target_derivation', 'The number of holdout results, since each evaluation '
        || 'inserts into both HOLDOUT_RESULTS and CHAT_LOG.'));
END IF;

-- ── Explanation quality: genuinely unmeasurable here ──────────────────────────
IF (:cv_table IS NOT NULL) THEN
  success_criteria := ARRAY_APPEND(:success_criteria, OBJECT_CONSTRUCT(
    'code', 'CONVO_EXPLANATION_QUALITY',
    'label', 'The assistant explains its reasoning clearly enough for a non-technical user',
    'why', 'Factual accuracy measures whether the NUMBER is right. It does not '
        || 'measure whether the EXPLANATION is clear, whether hallucinated entities '
        || 'appear alongside correct numbers, or whether the answer is useful to '
        || 'someone who does not know SQL.',
    'compare', '>=',
    'units', 'satisfactory explanations',
    'basis', 'BY_QUERY_ID',
    'target_derivation', 'Would require a human to read a sample of assistant '
        || 'responses and judge clarity, correctness of reasoning, and absence '
        || 'of hallucinated entities.',
    'pending_reason', 'Explanation quality cannot be measured by the holdout '
        || 'evaluation, which checks only whether the final numeric answer matches. '
        || 'It requires a human to read the full responses.',
    'resolves_when', 'A human reviews the responses in CHAT_LOG and judges whether '
        || 'the explanations are clear and free of hallucinated entities.'));
END IF;

-- ── Cost ──────────────────────────────────────────────────────────────────────
IF (:credit_cap > 0) THEN
  success_criteria := ARRAY_APPEND(:success_criteria, OBJECT_CONSTRUCT(
    'code', 'CONVO_COST_IN_BUDGET',
    'label', 'Measured steady-state cost stays inside your credit cap',
    'why', 'Each question costs ~0.01 credits in AI_COMPLETE tokens. A POC that '
        || 'cannot state its running cost cannot be approved for production.',
    'compare', '<=',
    'units', 'credits',
    'basis', 'BY_TAG',
    ...
    'target_derivation', 'Your CONVO_CREDIT_CAP setting, currently '
        || :credit_cap || ' credits.',
    'pending_reason', 'Warehouse credits and Cortex token charges reach '
        || 'ACCOUNT_USAGE on a delay, so nothing has been attributed to this '
        || 'run yet. This is an absence of data, not a cost of zero.',
...
ELSE
  success_criteria := ARRAY_APPEND(:success_criteria, OBJECT_CONSTRUCT(
    ...
    'na_reason', 'CONVO_CREDIT_CAP is 0, so no ceiling was declared for this run. '
...
END IF;
 ...
 || 'FROM s');
  ...
  IF (:tier = 'PRODUCTION') THEN
   ...
   || 'COMMENT = ''Cost attribution for Conversational Assistant. Query '
   ...
   || '''Conversational Assistant''');
    IF (:wh_ok) THEN
     ...
     || '.ONESHOT_SOLUTION = ''Conversational Assistant''');
    END IF;
    cost_detail := ARRAY_APPEND(:cost_detail,
   ...
   || 'ACCOUNT_USAGE.TAG_REFERENCES without asking us. Tags cost nothing.');
    ...
    ELSE
      notes := ARRAY_APPEND(:notes,
        'FAILURE NOTIFICATION SKIPPED: CONVO_NOTIFICATION_INTEGRATION is blank, so '
...
|| 'genuinely near-free -- this is not a missing measurement.'' '
 ...
 || '      RETURN ''REFUSED. This build was created with CONVO_ALLOW_SAMPLE_ACTIONS = '
 ...
 || 'created with CONVO_ALLOW_ACTIONS = FALSE, so nothing in the app can change '
 ...
 || '.V_BUILD_CONTEXT LIMIT 1); '
 || '    IF (NOT COALESCE(:enabled, FALSE)) THEN '
 || '      RETURN ''REFUSED. This build was created with CONVO_ALLOW_SAMPLE_ACTIONS = FALSE.''; '
 || '    END IF; '
 || '  ELSE '
 || '    enabled := (SELECT ACTIONS_ENABLED FROM ' || :tgt || '.V_BUILD_CONTEXT LIMIT 1); '
 || '    IF (NOT COALESCE(:enabled, FALSE)) THEN '
 || '      RETURN ''REFUSED. This build was created with CONVO_ALLOW_ACTIONS = FALSE.''; '
 || '    END IF; '
 || '  END IF; '
 || '  IF (:P_CONFIRM IS NULL OR UPPER(TRIM(:P_CONFIRM)) <> UPPER(TRIM(:P_CODE))) THEN '
 || '    RETURN ''REFUSED. Type the action code exactly to confirm it.''; '
...
|| 'before undoing it again.''; '
 ...
 || 'END');
  ...
  IF (ARRAY_SIZE(:actions) > 0) THEN
   ...
   || IFF(:allow_actions,
          'CONVO_ALLOW_ACTIONS is TRUE, so they are ARMED: a user of the dashboard can '
       || 'run them after typing the action code to confirm. Every attempt is recorded '
       || 'in ACTION_LOG.',
          'CONVO_ALLOW_ACTIONS is FALSE, so every button is inert and RUN_ACTION refuses. '
  ...
  IF (:prof_status = 'AVAILABLE') THEN
    ...
    ELSEIF (:prof_usable = 0 AND ARRAY_SIZE(:unusable) > 0 AND :n_dead < ARRAY_SIZE(:unusable)) THEN
      hard_block := 'The profile found NO usable column among the ' || ARRAY_SIZE(:unusable)
                 ...
                 || 'deterministic refusal from ' || 'CONVO' || '_MIN_FILL_PCT = ' || :min_fill
  ...
  ELSEIF (:prof_status NOT IN ('SYNTHETIC_INPUTS','BOUNDED_VALIDATED')) THEN
   ...
   || 'columns. Set CONVO_PROFILE = TRUE and re-run to close it.');
  END IF;
  ...
  BEGIN
    override_asked := (SELECT TRY_CAST($CONVO_OVERRIDE_REVIEW::VARCHAR AS BOOLEAN));
  ...
  review_prompt :=
    ...
    || 'SOLUTION: Conversational Assistant' || CHR(10)
    || 'TIER: ' || :tier || CHR(10)
    || 'TARGET: ' || :tgt || CHR(10)
 ...
 || CASE :review_verdict
      ...
      WHEN 'DO_NOT_PROCEED' THEN 'THE GATE IS CLOSED BY THIS VERDICT, even if '
        || 'CONVO_APPROVE is TRUE. To build anyway set CONVO_OVERRIDE_REVIEW = TRUE; '
 ...
 || 'BASE64_DECODE_STRING(''' || BASE64_ENCODE(LEFT(:review_raw, 8000)) || ''')');
  ...
  IF (:budget > 0 AND :cost_day > :budget) THEN
    res := (
      SELECT 0 AS step, 'BLOCKED — OVER BUDGET' AS action,
             ...
             || 'CONVO_BUDGET_CREDITS = ' || :budget || '. Nothing was created.' AS statement
      UNION ALL
  ...
  ELSEIF (:review_verdict = 'DO_NOT_PROCEED' AND :override_asked) THEN
   ...
   || 'CONVO_OVERRIDE_REVIEW = TRUE, so the build proceeded anyway. The verdict and '
  ...
  LET pk_md STRING :=
       '# ' || 'Conversational Assistant' || ' — discovery packet' || CHR(10) || CHR(10)
  ...
  LET pk_json STRING := TO_JSON(OBJECT_CONSTRUCT(
      'solution', 'Conversational Assistant', 'run_id', :run_id, 'tier', :tier,
    ...
    res := (
      ...
      UNION ALL SELECT 4, 'BUDGET',
             IFF(:budget > 0, 'ceiling ' || :budget || ' credits/day — within it',
                 'no ceiling set (CONVO_BUDGET_CREDITS = 0)')
             ...
             CASE WHEN :gate_closed_by = 'DETERMINISTIC CHECK' THEN :hard_block
                  WHEN :gate_closed_by = 'REVIEW VERDICT'
                    THEN 'The review returned DO_NOT_PROCEED. Read the findings above. '
                      || 'To build anyway set CONVO_OVERRIDE_REVIEW = TRUE.'
                  ELSE 'CONVO_APPROVE is FALSE. Nothing was created.' END
      UNION ALL
