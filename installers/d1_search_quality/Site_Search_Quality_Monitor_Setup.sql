...
SET SRCH_SOURCE_DISCOVERY_AI_APPROVED = TRUE;
...
BEGIN
  ...
  LET initial_discovery BOOLEAN := :source_discovery_mode = 'AUTO' AND $SRCH_APPROVE::BOOLEAN;
      ...
      ELSE
        ...
        ELSE
          ...
          LET inventory_query VARCHAR := 'WITH relations AS (SELECT t.TABLE_CATALOG AS DB, t.TABLE_SCHEMA AS SCH, t.TABLE_NAME AS TAB, t.TABLE_TYPE AS KIND, '
            ...
            || 'MAX(IFF(ARRAY_CONTAINS((t.TABLE_CATALOG||''.''||t.TABLE_SCHEMA||''.''||t.TABLE_NAME)::VARIANT,PARSE_JSON(?)),1000,0)) + MAX(IFF(REGEXP_LIKE(LOWER(t.TABLE_NAME), ''.*(orders|products|quality|queries|search|srch).*''),10,0)) + SUM(IFF(REGEXP_LIKE(LOWER(c.COLUMN_NAME), ''.*(orders|products|quality|queries|search|srch).*''),1,0)) AS RELEVANCE '
          ...
          ELSEIF ((:source_discovery_mode = 'PROPOSE' OR :initial_discovery) AND NOT $SRCH_SOURCE_DISCOVERY_AI_APPROVED::BOOLEAN) THEN
              ...
              discovery_proposal := (SELECT AI_COMPLETE(model => :discovery_model, prompt => :discovery_prompt,
                ...
                response_format => {'type':'json','schema':{'type':'object','additionalProperties':false,
                  'properties':{'mappings':{'type':'array','items':{'type':'object','additionalProperties':false,
                    'properties':{'setting':{'type':'string','enum':['SRCH_SEARCH_QUERIES_TABLE','SRCH_PRODUCTS_TABLE','SRCH_ORDERS_TABLE']},'table':{'type':'string'},'columns':{'type':'array','items':{'type':'string'}},'reason':{'type':'string'}},
...
BEGIN
    ...
    IF (UPPER($SRCH_SOURCE_DISCOVERY_MODE::VARCHAR) <> 'AUTO' OR :source_result:status::VARCHAR IN ('INVALID_SOURCE_SETTING','INVALID_SCOPE')) THEN
...
|| 'before undoing it again.''; '
 ...
 || 'BASE64_DECODE_STRING(''' || BASE64_ENCODE(LEFT(:review_raw, 8000)) || ''')');
  ...
  IF (:workload_blocked) THEN
    IF (:approved AND 'SEARCH_QUALITY_APP' <> '' AND 'd1_search_quality' <> '24_voice_of_customer' AND :app_build_end >= :app_build_start) THEN
